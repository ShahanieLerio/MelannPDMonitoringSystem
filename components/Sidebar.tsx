...
import { UserRole, Branch, canAccessPayments, canManageUsers } from '../types.ts';
import logo from '../assets/no bg.png';
import { useClientUpdates } from '../hooks/useClientUpdates.ts';
import { getPTPEscalationCases } from '../services/ptpEscalation.ts';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, activeTab, setActiveTab, role, onLogout, username, selectedBranch = Branch.ALL, migrationCount = 0, pendingUserCount = 0 }) => {
  ...
  const ptpEscalationCount = useMemo(() => getPTPEscalationCases(loans).length, [loans]);
  ...
  const noActivityIds = useMemo(() => loans.filter(l => l.remarks.length === 0 && hasActiveClientBalance(l)).map(l => l.id), [loans]);
  ...
  const legacyBranchRoles = [UserRole.NAVAL_USER, UserRole.ORMOC_USER];
  ...
  const fieldViewRoles = allOperationalRoles;
  ...
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: () => <span className="text-xl">📊</span>, roles: allOperationalRoles },
    { 
      id: 'loans', 
      label: 'Loan Grid', 
      icon: () => <span className="text-xl">📋</span>, 
      roles: allOperationalRoles,
      subItems: loanMaintenanceRoles.includes(role) ? [
    ...
    { 
      ...
      subItems: [
        ...
        { id: 'client-update-no-activity', label: 'No Commitments' }
      ]
    },
    { id: 'ptp-escalation', label: 'PTP Escalation', icon: () => <span className="text-[10px] font-black tracking-tight">PTP</span>, roles: fieldViewRoles },
    { 
      id: 'receive-payment', 
      label: 'Payments', 
      icon: () => <span className="text-xl">💰</span>, 
      roles: canAccessPayments(role) ? [role] : [],
      subItems: [
        { id: 'receive-payment-reverse', label: 'Reverse Payment' }
      ]
    },
    { id: 'collection-sheet', label: 'Collection Sheet', icon: () => <span className="text-xl">📄</span>, roles: fieldViewRoles },
    { id: 'dcr', label: 'DCR', icon: () => <span className="text-xl">📅</span>, roles: fieldViewRoles },
    { 
      id: 'reports', 
      label: 'Reports', 
      icon: () => <span className="text-xl">📉</span>, 
    ...
    { id: 'migration', label: 'JCASH Migration', icon: () => <span className="text-[10px] font-black tracking-tight">SYNC</span>, roles: dataMaintenanceRoles },
    ...
    { id: 'database', label: 'Backup & Restore', icon: () => <span className="text-xl">🗄️</span>, roles: adminToolRoles },
    { id: 'recycle-bin', label: 'Recycle Bin', icon: () => <span className="text-xl">♻️</span>, roles: dataMaintenanceRoles },
  ];
  ...
  return (
    ...
    `}</style>
    <aside
      className={`fixed inset-y-0 left-0 text-white transition-all duration-300 z-50 flex flex-col border-r border-emerald-200/10 shadow-2xl shadow-emerald-950/40 ${isOpen ? 'w-64' : 'w-16'}`}
      style={{
        background: 'radial-gradient(circle at 18% 0%, rgba(52,211,153,0.28) 0%, transparent 30%), radial-gradient(circle at 100% 48%, rgba(20,184,166,0.12) 0%, transparent 34%), linear-gradient(180deg, #0C3B2F 0%, #06261F 58%, #041B17 100%)'
      }}
    >
      <div className="px-3.5 flex items-center gap-3" style={{ paddingTop: '12px', paddingBottom: '8px' }}>
        <div
          className="shrink-0 flex items-center justify-center bg-transparent rounded-full shadow-xl shadow-black/25 overflow-hidden transition-all duration-300 cursor-pointer hover:scale-[1.03]"
          style={{ width: '52px', height: '52px' }}
          onClick={() => setIsOpen(!isOpen)}
        >
          <img src={logo} alt="Melann Lending" className="w-full h-full object-cover scale-110" />
        </div>
        {isOpen && (
          <div className="flex-1 min-w-0">
            <h1 className="font-black text-[14px] tracking-[0.3px] text-[#f8fafc] leading-tight drop-shadow-sm" style={{ whiteSpace: 'normal' }}>MELANN LENDING</h1>
            <p className="text-[10px] font-bold text-[#a7f3d0] uppercase tracking-[0.3px] leading-[1.15] mt-0.5" style={{ whiteSpace: 'normal', opacity: 0.9 }}>Past Due & Report<br />Monitoring System</p>
          </div>
        )}
      </div>
            ...
            <div key={item.id} className="flex flex-col">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-xl transition-all duration-300 relative group overflow-hidden ${
                  isActive
                    ? 'bg-white/18 text-white shadow-lg shadow-black/20 ring-1 ring-emerald-200/25'
              ...
              >
                ...
                <div className="relative flex items-center gap-2.5 w-full">
                  <div className={`shrink-0 flex h-7 w-7 items-center justify-center rounded-lg text-[16px] transition-all duration-300 ${
                    isActive ? 'bg-emerald-300/20 shadow-inner shadow-white/5 ring-1 ring-white/10' : 'bg-white/5 group-hover:bg-emerald-300/15'
                  } ${!isOpen && 'mx-auto'}`}>
                    <item.icon />
                  </div>
                  {isOpen && (
                    <span className="font-semibold text-[13px] whitespace-nowrap overflow-hidden text-left flex-1 flex justify-between items-center pr-1">
                      ...
                      {item.id === 'ptp-escalation' && ptpEscalationCount > 0 && (
                        <span className="shrink-0 bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-lg shadow-red-950/30 ml-2 ring-1 ring-white/20">
                          {ptpEscalationCount > 99 ? '99+' : ptpEscalationCount}
                        </span>
                      )}
                      {item.id === 'migration' && migrationCount > 0 && (
                        <span className="shrink-0 bg-amber-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-lg shadow-amber-950/30 ml-2 ring-1 ring-white/20">
                          {migrationCount > 99 ? '99+' : migrationCount}
                        </span>
                      )}
                      {item.id === 'users' && pendingUserCount > 0 && (
                        <span className="shrink-0 bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full shadow-lg shadow-red-950/30 ml-2 ring-1 ring-white/20">
                ...
                {!isOpen && item.id === 'users' && pendingUserCount > 0 && (
                  <span className="absolute right-1 top-1 rounded-full bg-red-500 px-1.5 py-0.5 text-[9px] font-black leading-none text-white shadow-lg shadow-red-950/30 ring-1 ring-white/40">
                    {pendingUserCount > 99 ? '99+' : pendingUserCount}
                  </span>
                )}
                {!isOpen && item.id === 'ptp-escalation' && ptpEscalationCount > 0 && (
                  <span className="absolute right-1 top-1 rounded-full bg-red-600 px-1.5 py-0.5 text-[9px] font-black leading-none text-white shadow-lg shadow-red-950/30 ring-1 ring-white/40">
                    {ptpEscalationCount > 99 ? '99+' : ptpEscalationCount}
                  </span>
                )}
      ...
      <div className="px-2.5 py-2 border-t border-emerald-200/10 bg-black/15 transition-colors duration-300">
        <ConfirmationModal
        ...
        />
        {isOpen && (
          <div className="mb-1.5 rounded-xl bg-white/10 px-3 py-1.5 ring-1 ring-white/10 shadow-inner">
