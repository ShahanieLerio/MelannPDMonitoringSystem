...
const DailyCollectionReport: React.FC<DailyCollectionReportProps> = ({ selectedBranch }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {/* Header */}
      <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-wrap gap-5 items-center justify-between transition-colors duration-300">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="bg-emerald-600 dark:bg-emerald-900/40 p-6 rounded-3xl shadow-xl shadow-emerald-900/20 dark:shadow-emerald-900/50 text-white flex items-center gap-5 transition-colors duration-300">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-5 transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)] gap-5 items-start">
            ...
            {data.collectorSummary.length > 0 ? (
              <table className="w-full text-left">
                <thead className="bg-slate-50/80 dark:bg-slate-900/50 text-slate-400 dark:text-slate-500 uppercase text-[10px] font-black tracking-widest transition-colors duration-300">
                  <tr>
                    <th className="px-5 py-3.5">Collector Name</th>
                    <th className="px-5 py-3.5 text-center">Accounts Collected</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50 transition-colors duration-300">
                  {data.collectorSummary.map((row, i) => (
                    <tr key={row.collector} className="group hover:bg-emerald-50 dark:hover:bg-slate-700/50 transition-all duration-300">
                      ...
                      <td className="px-5 py-3.5 text-center font-bold text-slate-600 dark:text-slate-400 transition-colors duration-300">{row.totalAccounts}</td>
                      <td className="px-5 py-3.5 text-right font-black text-emerald-600 dark:text-emerald-400 transition-colors duration-300">{formatCurrency(row.totalAmount)}</td>
                    </tr>
                  ))}
                  {/* Grand Total Row */}
                  <tr className="bg-slate-50 dark:bg-slate-900/50 border-t-2 border-slate-200 dark:border-slate-700 transition-colors duration-300">
                    <td className="px-5 py-3.5 font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest text-[11px] transition-colors duration-300">Grand Total</td>
                    <td className="px-5 py-3.5 text-center font-black text-slate-800 dark:text-white transition-colors duration-300">{data.grandTotalAccounts}</td>
                    <td className="px-5 py-3.5 text-right font-black text-emerald-700 dark:text-emerald-400 text-lg transition-colors duration-300">{formatCurrency(data.grandTotal)}</td>
                  </tr>
                </tbody>
              </table>
            ) : (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors duration-300 xl:sticky xl:top-6">
          <div className="p-5 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50 flex flex-wrap gap-4 items-center justify-between transition-colors duration-300">
            <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <select
                ...
                className="px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-colors duration-300"
              >
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50 font-medium transition-colors duration-300">
                  {filteredTransactions.map((t, i) => (
                    <tr key={`${t.orNumber}-${i}`} className="group hover:bg-emerald-50 dark:hover:bg-slate-700/50 transition-all duration-300">
                      <td className="px-4 py-3 text-slate-400 dark:text-slate-500 font-black text-[10px] transition-colors duration-300 group-hover:text-emerald-500 dark:group-hover:text-emerald-400">{i + 1}</td>
                      <td className="px-4 py-3">
                        <span className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tight text-xs transition-all duration-300 group-hover:text-emerald-700 dark:group-hover:text-emerald-400 group-hover:font-black group-hover:underline decoration-emerald-500/30 underline-offset-4">{t.borrowerName}</span>
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400 font-bold text-xs transition-colors duration-300">{t.collector}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400 font-mono text-[11px] transition-colors duration-300">{t.orNumber}</td>
                      <td className="px-4 py-3 text-right font-black text-emerald-600 dark:text-emerald-400 transition-colors duration-300">{formatCurrency(t.amount)}</td>
                      <td className="px-4 py-3 text-center text-slate-500 dark:text-slate-400 font-bold text-[11px] transition-colors duration-300">{t.date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
